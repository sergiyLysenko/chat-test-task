...
const ChatLayout = memo(() => {
  const { socket, reconnectConnection, disconnectConnection } = useChatConnection();

  return <Chat disconnectConnection={disconnectConnection} reconnectConnection={reconnectConnection} socket={socket} />
})

const Chat = ({ socket, reconnectConnection, disconnectConnection }: { socket: Socket<DefaultEventsMap, DefaultEventsMap>, reconnectConnection: VoidFunction, disconnectConnection: VoidFunction }) => {
  ...
  return (
    <Box as="section">
      <Text mb='20px' textAlign='center' as='h1'>Simple Chat</Text>
      <Center gap='2px' backgroundColor='thistle' w='100%' h='500px'>
        ...
        {!selectedRoom && <Center w='70%' h='100%' backgroundColor='tomato'>Select room</Center>}
      </Center>
      <Box>
